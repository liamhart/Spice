...
def index_exp(node):
    ...
    if len(i) == 2:
        (type, val) = ('list', state.symbol_table.lookup_sym(name)[1][int(i[0])*2 + 1:(int(i[1]) + 1)*2 + 1:2])
    if len(i) == 1:
        (type, val) = (state.symbol_table.lookup_sym(name)[1][int(i)*2], state.symbol_table.lookup_sym(name)[1][int(i)*2 + 1])
        
    return val

#########################################################################
def int_index_exp(node):
    (INTINDEX, integer, i) = node
    assert_match(INTINDEX, 'intindex')

    foo = str(node[1])
    bar = []
    for i in range(int(node[2][1])):
        bar.append(foo[i])
    bar[::-1]
    foo = ''.join(str(e) for e in bar)
    return int(foo)
...
#########################################################################
def bracket_exp(node):
    
    (BRACK, exp) = node
    assert_match(BRACK, 'bracket')
